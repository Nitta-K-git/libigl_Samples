...
int main(int argc, char *argv[])
{
  using namespace Eigen;
  using namespace std;
  igl::readOBJ("./decimated-max.obj",V,F);
  U=V;
  // S(i) = j: j<0 (vertex i not in handle), j >= 0 (vertex i in handle j)
  VectorXi S;
  igl::readDMAT("./decimated-max-selection.dmat",S); // 頂点数x1の行列データ、-1,0,1,2
  igl::colon<int>(0,V.rows()-1,b); // low,high,ret,(step=1)
  b.conservativeResize(stable_partition( b.data(), b.data()+b.size(), // dmatの値が正のものを前に持ってくる。相対的な位置関係は維持 https://programming-place.net/ppp/contents/cpp/library/022.html
   [&S](int i)->bool{return S(i)>=0;})-b.data()); // stable_partitionの戻り値は条件不一致の最初の要素なので、データ頭を引くとインデックスが正の数の要素数になる
